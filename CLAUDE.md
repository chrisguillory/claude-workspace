...
**Missing-layer detection.** When the same concern duplicates across higher layers, a lower layer is missing. Two callsites is the signal to consider extraction; three makes it overdue. Past example (now retired): `_nfkc_casefold` duplicated between `orchestrator.py` and `bluetooth.py` triggered extraction to `cc_lib/utils/unicode_match.py` as soon as the second caller emerged. Live example: W8 and W14 both stem from "our dispatch shape is shell-script-as-message" — one cause, two symptoms across the orchestrator, retired together by a typed-RPC layer.
