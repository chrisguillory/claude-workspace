...
- **Evidence artifacts are deliverables.** The screenshots, before/after captures, and verbatim outputs that convinced *you* are what will convince the reviewer — and the *before* state becomes expensive to re-stage the moment the fix lands. Capture both states at verification time, keep the files, and pipe them into the deliverable (a PR's test plan — see create-pr's evidence step, `~/claude-workspace/scripts/gh-upload.py` — a doc, the next session). A verdict whose evidence died in a temp dir reads as *"trust me, I watched it work."*
...
| **empirical residue**       | the evidence artifacts a verification leaves behind — captured for yourself, delivered to the reviewer |
