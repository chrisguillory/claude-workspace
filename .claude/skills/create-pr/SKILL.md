...
- **Screenshots / visual evidence** — for rendering, UI, or layout work, pixels out-evidence prose. PR bodies render images **only from URLs** (base64 data URIs are stripped; local paths don't upload), so upload them first:
  ...
  Browser-session cookie replay against GitHub's own upload endpoint (`gh` tokens can't reach it; the gist API rejects binaries outright) → permanent first-party `user-attachments` URLs, identical to drag-dropping into the web editor. Repo context derives from the cwd (`--repo owner/name` to override); images print as `![name](url)` embed lines, other artifacts as `[name](url)` links — paste directly, pairing before/after captures in a table or side-by-side inside `<details>`. A 422 means the saved browser session expired — re-save it per the script's module docstring. Uploading is auto-allowed like `publish-plan.py`, which makes the check before it the only gate: **the public-repo PII gate applies to pixels** — screenshots carry window chrome, document titles, and content vocabulary, so re-capture with generic content rather than upload-then-scrub (user-attachments are permanent; there is no delete).
