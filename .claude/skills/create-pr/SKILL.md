...
allowed-tools:
  ...
  - "Bash(~/claude-workspace/scripts/gh-upload.py:*)"
---
...
- **Visual evidence via `~/claude-workspace/scripts/gh-upload.py`** — screenshots/renders upload to permanent first-party `user-attachments` URLs (same as web drag-drop); the pixel-level PII check happens before uploading, by re-capturing clean.
...
- **Screenshots left on disk** — arguing a visual change in prose while the captures sit in a temp dir; upload via `~/claude-workspace/scripts/gh-upload.py` and embed them.
