...
allowed-tools:
  ...
  - "Bash(.claude/skills/create-pr/publish-evidence.py:*)"
...
- **Screenshots / visual evidence** — for rendering, UI, or layout work, pixels out-evidence prose. PR bodies render images **only from URLs** (base64 data URIs are stripped; local paths don't upload; the web editor's drag-drop endpoint rejects `gh` tokens), so host them first:

  ```bash
  .claude/skills/create-pr/publish-evidence.py <slug> <image>...
  ```

  Idempotent per slug (same local-store pattern as the plan gist): first run creates a secret gist, re-runs update it in place; it prints `![name](https://gist.githubusercontent.com/…)` lines that embed directly in the body. Pair before/after captures in a table or side-by-side inside `<details>`. Publishing is auto-allowed like `publish-plan.py`, which makes the check before it the only gate: **the public-repo PII gate applies to pixels** — screenshots carry window chrome, document titles, and content vocabulary, so re-capture with generic content rather than publish-then-scrub (gist history keeps every revision).
...
- **Visual evidence via gist** — screenshots/renders go through `publish-evidence.py` (idempotent slug store, gist raw URLs); the pixel-level PII check happens before publishing, by re-capturing clean.
...
- **Screenshots left on disk** — arguing a visual change in prose while the captures sit in a temp dir; publish via `publish-evidence.py` and embed them.
