...
async def download_resource(url: str, output_filename: str) -> dict:
    ...
    if response.status >= 400:
        raise fastmcp.exceptions.ToolError(f'Download failed with status {response.status}: {url}')
...
async def get_aria_snapshot(selector: str, include_urls: bool = False, timeout: int = 1000) -> str:
    ...
    if not include_urls:
        ...
        snapshot_yaml = yaml.dump(filtered_data, default_flow_style=False, sort_keys=False)
