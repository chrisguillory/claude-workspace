...
class EventStream(Generic[T]):
    ...
    def __init__(
        ...
        self.generator = stream_events(response, decoder, sentinel, data_required=data_required)
...
class EventStreamAsync(Generic[T]):
    ...
    def __init__(
        ...
        self.response = response
        self.generator = stream_events_async(response, decoder, sentinel, data_required=data_required)
        self.client_ref = client_ref
        self._closed = False
...
def _parse_event(
    *,
    ...
    out = None
    if publish:
        out_dict = {k: v for k, v in asdict(event).items() if v is not None or (k == 'data' and data)}
