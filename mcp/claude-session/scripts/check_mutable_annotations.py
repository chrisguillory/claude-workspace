...
"""Check for mutable collection types in model annotations.

This script enforces the use of immutable/abstract collection types
(Sequence, Mapping, Set) instead of mutable concrete types
(list, dict, set) in Pydantic model field and method parameter annotations.
...
Design Philosophy:
    ...
    - Escape hatch: # noqa: mutable-type to suppress specific violations

Usage:
    ./scripts/check_mutable_annotations.py <files...>
    ./scripts/check_mutable_annotations.py .          # All .py files in cwd
...
class Violation:
    """A single mutable type violation."""
...
class MutableTypeChecker(ast.NodeVisitor):
    ...
    def _check_annotation(self, node: ast.expr, lineno: int, context: str) -> None:
        """Check if an annotation contains forbidden mutable types."""
        # Check for noqa comment first
        if self._has_noqa(lineno):
            return

        mutable_type = self._find_mutable_type(node)
        if mutable_type:
            ...
            self.violations.append(
                Violation(
                    ...
                    suggestion=self.SUGGESTIONS.get(mutable_type, 'an immutable type'),
                )
            )

    def _find_mutable_type(self, node: ast.expr) -> str | None:
        """Recursively find if annotation contains a forbidden mutable type.

        Returns the mutable type name if found, None otherwise.
        """
        if isinstance(node, ast.Name):
            ...
            if container_name in TRANSPARENT_CONTAINERS:
                return self._find_mutable_in_slice(node.slice)
            ...
            # Union type: X | Y
            left_violation = self._find_mutable_type(node.left)
            if left_violation:
                return left_violation
            return self._find_mutable_type(node.right)
            ...
            for element in node.elts:
                violation = self._find_mutable_type(element)
                if violation:
                    return violation
            return None
    ...
    def _find_mutable_in_slice(self, node: ast.expr) -> str | None:
        """Check for mutable types in a subscript slice."""
...
def format_violation(v: Violation) -> str:
    ...
    return (
        ...
        f"Mutable type '{v.mutable_type}' in {v.context} annotation\n"
        f'    {v.source_line}\n'
        f'    Suggestion: Use {v.suggestion}\n'
        f'    Silence with: # noqa: mutable-type'
    )
...
def main() -> int:
    ...
    if all_violations:
        ...
        print(f'Found {len(all_violations)} mutable type violation(s) in {file_count} file(s).')
        return 1
