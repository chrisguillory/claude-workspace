...
| W9  | NFKC + smart-quote/NBSP fold for `--output` matching                                     | `orchestrator.py` and `bluetooth.py` (`_nfkc_casefold`)                  | Core Audio stores renamed devices with U+2019 (curly apostrophe); keyboards type U+0027                   | Apple                                                   | Folding is the correct fix; **TODO**: extract duplicated helper to `cc_lib/unicode_match.py`                  |
...
4. **R1 — Adopt `sox_ng 14.8.0` as roc-toolkit's bundled libsox**. The lever. Requires rebuilding roc-toolkit with sox_ng substituted into its scons 3rdparty build (libsox is statically linked into roc-send/roc-recv per `otool -L`). Deletes W1, W2, simplifies W3. Empirically test the full mesh.
