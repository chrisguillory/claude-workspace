...
class BrowserService:
    ...
    @tool_registry.register_tool
    async def screenshot(self, filename: str, full_page: bool = False, trigger_lazy: bool = True) -> str:
        ...
        if full_page:
            ...
            screenshot_data = await _capture_full_page(driver, self, trigger_lazy=trigger_lazy)
    ...
    async def export_har(
        ...
        if not logs:
            ...
            empty_har = {
                'log': {
                    'version': '1.2',
                    'creator': {'name': 'selenium-browser', 'version': '1.0'},
                    'entries': [],
                },
            }
...
async def _warm_up_lazy_content(driver: webdriver.Chrome, service: BrowserService) -> None:
    """Scroll the full page so lazy / deferred content renders before a capture.

    A single CDP captureBeyondViewport spans the full height but never scrolls, so content that
    renders only when scrolled into view — GitHub's Mermaid/math (deferred cross-origin iframe
    enrichment), lazy-loaded images — stays blank below the fold. Step top→bottom dwelling at each
    stop so IntersectionObservers fire and deferred iframes start loading, then wait for the page
    to settle (so the capture isn't taken mid-render) before returning to the top.
    """
    ...
    vh = int(metrics['vh'])
    # Half-viewport stride overlaps stops so every observer fires; the final stop lands on the
    # bottom. Dwell at each stop long enough for the observer callback + GitHub's enrichment to fire
    # (a sub-200ms dwell is too fast — the deferred iframe never starts loading).
    stride = max(vh // 2, 1)
    ...
    await _await_page_settled(driver, service)
    await asyncio.to_thread(driver.execute_script, 'window.scrollTo(0, 0);')
    await asyncio.sleep(0.3)


async def _await_page_settled(driver: webdriver.Chrome, service: BrowserService, timeout_s: float = 25.0) -> None:
    ...
    while asyncio.get_running_loop().time() < deadline:
        await service.wait_for_network_idle(timeout=2000)
...
_TILE_DWELL_LAZY_S = 1.2  # per-tile dwell when trigger_lazy: a re-deferred cross-origin iframe re-renders in view
_TILE_DWELL_FAST_S = 0.2  # per-tile dwell otherwise — no waiting on lazy content
...
async def _capture_full_page(driver: webdriver.Chrome, service: BrowserService, *, trigger_lazy: bool) -> bytes:
    """Full-page PNG via scroll-and-stitch — the only method that captures lazy cross-origin iframes.

    A single CDP captureBeyondViewport spans the full height but never moves the visual viewport, so
    cross-origin (out-of-process) iframe content that renders only when scrolled into view — GitHub's
    Mermaid/math in viewscreen iframes — comes back blank, and GitHub *re-defers* those iframes once
    scrolled away. So we capture the page in overlapping viewport tiles, each taken while its content
    is in view and rendered, then stitch them. Sticky/fixed elements are flattened and scrollbars
    hidden first (so they neither duplicate across tiles nor bake a scrollbar into each); the page is
    restored afterward.
    """
    await asyncio.to_thread(driver.execute_script, _PREPARE_STITCH_JS)
    try:
        if trigger_lazy:
            await _warm_up_lazy_content(driver, service)
