...
#
# Setup:
#   claude mcp add --transport stdio selenium-browser-automation -- uv run --script "$(git rev-parse --show-toplevel)/selenium-browser-automation-mcp.py"
...
"""
Architecture: All tools operate on singleton _driver (shared browser session state).
...
async def lifespan(server):
    ...
    try:
        ...
        # Cleanup: driver.quit() closes browser and all windows
        global _driver
        if _driver:
            await asyncio.to_thread(_driver.quit)
            _driver = None
...
async def _close_browser():
    """Tear down browser. Reset all global state."""
    global _driver
...
async def get_browser():
    ...
    _driver = await asyncio.to_thread(webdriver.Chrome, options=opts)
...
@mcp.tool(
    annotations=ToolAnnotations(title='Navigate to URL', destructiveHint=False, idempotentHint=True, openWorldHint=True)
)
async def navigate(url: str, fresh_browser: bool = False) -> NavigationResult:
    ...
    print(f'[navigate] Navigating to {url}' + (' (fresh browser)' if fresh_browser else ''))

    if fresh_browser:
        await _close_browser()

    driver = await get_browser()

    # Navigate (blocking operation)
    await asyncio.to_thread(driver.get, url)

    print(f'[navigate] Successfully navigated to {driver.current_url}')
...
async def get_page_content(
    format: Literal['text', 'html', 'markdown'], ctx: Context, selector: str | None = None, limit: int | None = None
) -> str:
...
if __name__ == '__main__':
    print('Starting Selenium Browser Automation MCP server')
    print('Note: This server uses CDP stealth injection to bypass bot detection')
    mcp.run()
